...
module.exports = async (req, res) => {
  ...
  if (!tag || !description) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE_FOR_CREATE_PROJECT));
  if (description.length < 10 || description.length > 50) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.DESC_LENGTH_OUT_OF_LIMIT));
